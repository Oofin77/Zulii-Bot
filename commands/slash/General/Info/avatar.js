...
module.exports = {
  ...
  type: 1,
  options: [{ name: "target", description: "Select the target.", type: 6 }],
  ...
  async displayAvatar(interaction, userID) {
    ...
    const member = await interaction.guild.members.fetch(
      memberExists ? user.id : interaction.user.id
    );
    ...
    const size_menu = new discord.ActionRowBuilder().addComponents(
    ...
    const format_menu = new discord.ActionRowBuilder().addComponents(
      new discord.StringSelectMenuBuilder()
        .addOptions(
          formats.map((format) => ({
    ...
    const change_menu = new discord.ActionRowBuilder().addComponents(
      new discord.StringSelectMenuBuilder()
    ...
    const e = await interaction.editReply({
      embeds: [embed],
    ...
    e.createMessageComponentCollector({ idle: 3e5 })
      .on("collect", async (i) => {
        ...
        if (i.user.id === interaction.user.id) {
          if (type === "avatar") {
            ...
            if (i.customId === "change_menu") {
              if (i.values[0] === "server") {
                ...
                await change_menu.components[0]
                  .setOptions({ label: "User", value: "user", emoji: "🔁" })
                ...
                embed.setFields({
                  ...
                  value: [
                    `> **${animatedLanguage}: ${
                      String(user.avatarURL()).includes(".gif") ? "✅" : "❌"
                    }**`,
                    `> **${serverAvatarLanguage}: ${"❌"}**`,
                  ]
                    .filter(function (e) {
                ...
                await change_menu.components[0]
                  .setOptions({ label: "Server", value: "server", emoji: "🔁" })
                  .setPlaceholder(`${displayAvatarLanguage}: User`);
                await format_menu.components[0]
                  .setOptions(
                    formats2.map((format) => ({
          ...
          await i.followUp({
            content: `${errorLanguage.replace("{X}", X)}`,
            ephemeral: true,
          });
        }
      })
